...
export function MutualConfirmationPanel({ confirmation, mode = 'hr' }: MutualConfirmationPanelProps) {
  return (
    <div className="story-block mutual-panel">
      <div className="risk-review-head">
        ...
        <Badge tone={confirmation.candidateConfirmedItems.length >= 3 ? 'green' : 'amber'}>
          {confirmation.candidateConfirmedItems.length >= 3 ? '确认充分' : '需补充确认'}
        </Badge>
      </div>
