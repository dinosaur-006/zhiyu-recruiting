...
import type { Job, JobInput, SituationalParams } from '../../types';
...
export function JobNew() {
  ...
  const [sitParams, setSitParams] = useState<SituationalParams | null>(null);
  ...
  const submit = async (event: FormEvent) => {
    ...
    setAiNotice('');
    try {
      const result = await addJobWithAi(form, sitParams ?? undefined);
    ...
    <main className="page">
      ...
      <div className="editor-grid">
        <form className="panel form-panel" onSubmit={submit}>
          {/* Phase 3: 造物主引擎 */}
          <details open>
            <summary>造物主引擎 — DNA 逆向提取</summary>
            <GenesisEngine
              jobTitle={form.title}
              jobDepartment={form.department}
              existingJD={[form.responsibilities, form.requirements, form.challenges, form.teamInfo, form.workload].filter(Boolean).join('\n')}
              onParamsChange={setSitParams}
            />
          </details>
          ...
          <button className={`primary-button full${isGenerating ? ' pulse-glow' : ''}`} type="submit" disabled={isGenerating}>
        ...
        <aside className="panel ai-side">
          ...
          {createdJob ? (
            ...
            <div className="ai-skeleton-group">
              ...
              <div className="skeleton" style={{ height: '16px', width: '35%', marginBottom: '12px' }} />
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <div className="skeleton" style={{ height: '24px', width: '65px', borderRadius: '9999px' }} />
                <div className="skeleton" style={{ height: '24px', width: '80px', borderRadius: '9999px' }} />
                <div className="skeleton" style={{ height: '24px', width: '55px', borderRadius: '9999px' }} />
              </div>
              <div className="skeleton" style={{ height: '16px', width: '35%', marginBottom: '12px' }} />
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <div className="skeleton" style={{ height: '24px', width: '70px', borderRadius: '9999px' }} />
                <div className="skeleton" style={{ height: '24px', width: '60px', borderRadius: '9999px' }} />
                <div className="skeleton" style={{ height: '24px', width: '90px', borderRadius: '9999px' }} />
              </div>
              <div className="skeleton" style={{ height: '16px', width: '35%', marginBottom: '12px' }} />
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <div className="skeleton" style={{ height: '24px', width: '55px', borderRadius: '9999px' }} />
                <div className="skeleton" style={{ height: '24px', width: '75px', borderRadius: '9999px' }} />
              </div>
